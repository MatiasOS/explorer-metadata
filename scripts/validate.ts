...
const orgSchema = JSON.parse(
  fs.readFileSync(path.join(ROOT_DIR, "schemas/organization.schema.json"), "utf-8")
);
const supporterSchema = JSON.parse(
  fs.readFileSync(path.join(ROOT_DIR, "schemas/supporter.schema.json"), "utf-8")
);
  ...
  if (fs.existsSync(tokensDir)) {
    ...
    for (const chainDir of chainDirs) {
      if (chainDir.isDirectory()) {
        ...
        const files = fs.readdirSync(chainPath).filter((f) => f.endsWith(".json"));
      ...
      for (let i = 0; i < content.organizations.length; i++) {
        ...
        if (!isValid) {
          ...
          if (org.logo) {
            ...
            if (!fs.existsSync(logoPath)) {
              additionalErrors.push(
                `Referenced logo not found: ${org.logo}`
              );
            }
          }
          if (additionalErrors.length > 0) {
